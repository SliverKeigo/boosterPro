...
export function buildSupplementData(body: any, mode: 'create' | 'update') {
  ...
  const profiles = (customerProfiles as any[])
    .filter((r) => r.specialty || r.description || r.attachmentUrl?.length)
    .map((r) => ({ specialty: r.specialty || null, description: r.description || null, attachmentUrl: Array.isArray(r.attachmentUrl) ? r.attachmentUrl : [] }))
