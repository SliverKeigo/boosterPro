...
describe('buildSupplementData - 字段映射与清洗', () => {
  ...
  it('create：demandUpdates / customerProfiles 过滤空记录并转换', () => {
    ...
    expect(out.customerProfiles.create[0].attachmentUrl).toEqual(['/api/files/p.pdf'])
    expect(out.customerProfiles.create[1]).toEqual({ specialty: '只有专长', description: null, attachmentUrl: [] })
  })
