...
> 状态：**全部完成** ✅ —— ① 组模块、② 工作计划三层、③ 全模块导入（9 扁平模块 + 工作计划矩阵导入）均已落地、验证、提交。
> 日期：2026-06-05
...
| 阶段 | 状态 | 提交 | 说明 |
| --- | --- | --- | --- |
| ① 组模块 | ✅ 完成 | b5a813b | schema(Group+User.groupId)、组 CRUD API、组管理页+菜单、权限助手(getMyLedGroupId/assertCanWriteWorkPlan)、permissions/my 回 groupId+ledGroupId。A6 已核查：225 生产 work_plans=0，重建安全。 |
| ② 工作计划三层 | ✅ 完成 | 7f14ad8 | WorkPlan/WorkPlanItem/WorkPlanAssignment 三表；GET 按组过滤、写操作 assertCanWriteWorkPlan 守卫；工作计划页重构为「明细行+组员日期矩阵」编辑器+「交付中心」菜单。 |
| ③ 全模块导入 | 🟦 9/9 扁平模块完成 | 9469cd6 / 7991751 / 8182ac3 / f0c72b5 | 通用导入引擎(importServer)+ 配置(importConfigs/importColumns)+ /api/import/[resource] + ImportModal + BoostTable importResource。九大列表模块(人才库/候选人/客户/需求/客户补充/联系人/商机/合同/知识库)均支持导出→改→导入(id-upsert + 关系按名称反查 + 子表 JSON + 整文件事务 + 必填校验 + omitIfEmpty)。**待做：工作计划矩阵导入**(见下)。 |

### ③ 工作计划「主从-矩阵」导入 ✅（提交 e61da68）

已用**扁平表 + 单列「组员分配」**实现（避开动态成员列，且与子表「可读文本」格式一致）：
- 导出（自定义 `onExport`）：每行 = 一个明细行，列含 `周计划id/明细id/组/周/策略/客户/岗位/进展/岗位开放时间/是否例行寻猎` + **组员分配**（每行「组员名=日期」，如 `张伟=6.2`）。
- 导入 `/api/work-plans/import`（`src/lib/workPlanImport.ts`）：按「周计划id / 组+周」聚合多行 → 重建 `WorkPlan + items + assignments`；客户/岗位按名称反查、组员名在该组内解析为 userId；有 id 更新（deleteMany+重写）、无 id 新增；参与度自动算；每个计划按其组做组长守卫；空明细行跳过。

> **子表导出格式**：经反馈由 JSON 改为**可读文本**（每行一条、字段用 ` | ` 分隔，表头标字段顺序；最后字段吸收多余 `|`），见提交 19f9a9f。

每阶段门控均通过：tsc 0 错 / lint 干净 / 全量测试通过 / `npm run build` 成功。本地 dev 库已建好 groups、work_plans(重建)、work_plan_items、work_plan_assignments。

> **③ 导入的补充决策（用户确认）**：① 候选人等带子表的模块**子表也要导入** —— 导出时每个子表占一列、单元格存该子表行的 **JSON 数组**（如 `[{"date":"2026-01-01","content":"…"}]`），导入按此规范解析回子表（主表 deleteMany 子表后重建）。② 部分行校验失败 → **整文件事务，有错全不写**，返回错误明细（行号+原因）。
