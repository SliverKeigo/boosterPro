...
> 状态：**执行中** —— ① 组模块 ✅、② 工作计划三层 ✅、③ 全模块导入 🟦9/9 扁平模块完成（工作计划矩阵导入待做）
> 日期：2026-06-05
...
工作计划与扁平模块不同，**不适配通用引擎**，需专用实现（建议作为聚焦后续）：
- **动态列**：导出列含「按组成员动态生成」的列（A员工~F员工＝该组成员名），通用 `IMPORT_COLUMNS` 的静态列表达不了；截图是「**单个组×单周**」一张表，而非扁平列表。
- **主从-矩阵重建**：导入需按 workPlanId（或 组+周）把多行聚合成一个 WorkPlan，每行→WorkPlanItem，非固定列（成员名）→ 解析成员名为 userId → WorkPlanAssignment。
- 建议：单独的「按组导出该组本周计划」+ 专用 `/api/work-plans/import`（聚合行重建三层）。当前工作计划以 UI 编辑器为主，批量导入为低频补充。
