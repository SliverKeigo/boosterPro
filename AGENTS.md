...
- **列表行操作＝「详情」入口（不是直接编辑）**：行内按钮统一是「详情」(`Eye` 图标，所有能看到该行的人都可点)，点开是**复用编辑表单的只读态**——页面加 `mode: 'view'|'edit'` 状态，`openDetail(r)` 置 `view`、`openCreate()` 置 `edit`；`<Modal>` 传 `readOnly={mode==='view'}`（Modal 会把 children 包进 `fieldset disabled + pointer-events-none` 禁用一切控件）+ `onEdit={can(RES,'EDIT') && isOwner(editing) ? ()=>setMode('edit') : undefined}`（只读模式下底部据此显示「编辑」按钮，点击解锁为可编辑、底部变「保存」）。`title` 用 `mode==='view' ? 'X详情' : editing ? '编辑X' : '新增X'`。**删除**按钮仍在行内、仍按 `DELETE + isOwner` 收口。`onEdit` 的权限口径**随页面而变**：业务列表用 `can(RES,'EDIT') && isOwner(editing)`；**系统管理类（用户/部门/角色/组/权限/字典/提示词）用 `isAdmin`**；**工作计划用 `can('WORK_PLAN','EDIT')`**（已纳入权限体系、无行级归属）。字典是「类型 + 项」两套独立流程，各有自己的 `modeType/modeItem`。**全部列表/管理页（含系统管理）均已套用此「详情→编辑」模式**，新增模块照此办理。
...
- **导入已恢复（导出→改→导入回写）**：`BoostTable` 传 `importResource='<RESOURCE_KEY>'` 即开启该列表的「可回导导出 + 导入」。导出走 `src/lib/importColumns.ts` 的可回导列（首列 `id`、关系列出名称、子表列出 JSON 数组）；导入走 `/api/import/[resource]`（`src/lib/importServer.ts` 引擎 + `importConfigs.ts` 配置）：**有 id 更新（`assertRowWritable`）、无 id 新增（置 `createdById`）；关系列按名称唯一反查 id（重名/查无→该行报错）；子表 JSON 解码为 nested create；整文件事务（任一行校验失败则整批不写）；必填校验、`omitIfEmpty`（NOT NULL+默认值列空则省略）**。已接入九大业务列表模块；`resources.ts` 的 `IMPORT` 权限已恢复可配置。新增模块导入＝在 `importConfigs.ts`(服务端字段/关系/子表) + `importColumns.ts`(客户端导出列，表头须一致) 各加一项 + 页面传 `importResource`。**工作计划不提供导入/导出**（`WORK_PLAN` 资源无 IMPORT/EXPORT 动作，见 `resources.ts` 的 `NO_IO_RESOURCES`）。
- **组 / 工作计划**：「组」挂在部门之下（`Group.departmentId` + `leaderId` 组长 + `User.groupId` 成员，管理员在「系统管理→组管理」维护）。「工作计划」是三层（`WorkPlan` 周计划【全公司**一周一条**，`weekStart` 唯一、不绑组】→ `WorkPlanItem` 明细行【带 `groupId` 标记来源组，用于**按组分页**；客户/岗位只读】→ `WorkPlanAssignment` 组员日期矩阵【`planDates` 存 **JSON 日期数组字符串**】）。**鉴权纳入权限体系**：业务资源 `WORK_PLAN`（VIEW 看全部、CREATE/EDIT/DELETE，**无行级归属、不按部门/组过滤**）。新增弹窗走 `GET /api/work-plans/all-setup`（全员=矩阵列 + 各组在招需求按组生成明细 + 上周「同客户同岗位」进展自动带入），明细**全自动生成、不可手动加行**；前端组件 `MultiDatePicker`（日期多选）、`Modal size="full"`（全屏）。`/api/permissions/my` 仍回 `groupId`/`ledGroupId`（`getMyGroupId`/`getMyLedGroupId` 保留；`assertCanWriteWorkPlan` 已退役）。
...
## 权限系统
- 资源 / 动作常量在 `src/lib/resources.ts`（前后端共享，**单一事实源**；新增资源/动作只改这里 + schema `created_by_id` + route 守卫）。十一资源：CANDIDATE/REQUIREMENT/CLIENT_SUPPLEMENT/**CUSTOMER_CONTACT**/TALENT_POOL/OPPORTUNITY/CUSTOMER/CONTRACT/KNOWLEDGE/REPORT/**WORK_PLAN**；六动作：VIEW/CREATE/EDIT/DELETE/IMPORT/EXPORT。**WORK_PLAN 无导入导出**（`NO_IO_RESOURCES`，权限矩阵据此裁剪 IMPORT/EXPORT 列）；REPORT/WORK_PLAN「看全部」不参与部门可见性（`departments` 页 `VISIBILITY_RESOURCES` 已排除）。
