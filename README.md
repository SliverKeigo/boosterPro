...
- 客户需求管理：JD 录入 + AI 岗位画像分析（联网、动态条目填入子表）；**岗位状态多选**；登记日期新增 / 修改记录时自动取今天（仍可手改）
- 客户补充信息、人才储备库
...
>
> **服务商可切**（由 `AI_PROVIDER` 分流，见 `src/lib/ai.ts`）：
> - **OpenAI / 豆包（默认，走 Responses API + `web_search`）**：只改 `.env` 即可。豆包（火山方舟 Ark）填 `OPENAI_BASE_URL=https://ark.cn-beijing.volces.com/api/v3`、`OPENAI_API_KEY=<Ark Key>`、`OPENAI_MODEL=doubao-seed-1-6-250615`（需支持联网）；Ark 的 Responses API 与 `web_search` 同 OpenAI 同形，无需改代码。
> - **DeepSeek（走 Anthropic 兼容接口，联网由 DeepSeek 后端托管、无需外接搜索）**：设 `AI_PROVIDER=deepseek`、`ANTHROPIC_BASE_URL=https://api.deepseek.com/anthropic`、`ANTHROPIC_API_KEY=<DeepSeek Key>`、`AI_MODEL=deepseek-v4-flash`。代码已内置该分支（`@anthropic-ai/sdk` 的 `messages.create` + 托管 `web_search` 工具，工具版本可用 `ANTHROPIC_WEB_SEARCH_TYPE` 覆盖）。
